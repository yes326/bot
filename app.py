...
async def send_business_reply(message, text, bcid):
    """Отправка ответа в бизнес-чат или обычный чат"""
    if bcid:
        try:
...
# --- Обработка бизнес-сообщений и команд ---
...
async def handle_business_logic(message: types.Message):
    try:
        ...
        # 1. Сравниваем ID с OWNER_ID или owner_id
        # 2. В бизнес-сообщениях от владельца message.from_user.id всегда совпадает с ID владельца
        is_owner = (msg_from == OWNER_ID or msg_from == owner_id)

        # Если сообщения с бизнес-подключением пришли, но from_user не совпадает — проверяем по чату
        if not is_owner and not bcid and message.chat.type == "private":
            is_owner = (msg_from == OWNER_ID)
        ...
        if message.text and message.text.startswith("."):
            ...
            # Выполняем команды ТОЛЬКО если их пишет владелец
            if not is_owner:
            ...
            if cmd == ".mute":
                ...
                await send_business_reply(message, f"🔇 Собеседник замучен на {m} мин", bcid)
                return

            elif cmd == ".unmute":
                mutes.pop(t, None)
                await try_delete(message)
                await send_business_reply(message, "🔊 Мут снят", bcid)
                return
                ...
                get_stats(t)["warns"] += n
                await try_delete(message)
                await send_business_reply(message, f"⚠️ Предупреждений у собеседника: {warns[t]}/{WARN_LIMIT}", bcid)
                if warns[t] >= WARN_LIMIT:
                    mutes[t] = datetime.now() + timedelta(minutes=WARN_MUTE_MINUTES)
                    await send_business_reply(message, f"🔇 Достигнут лимит варнов! Мут на {WARN_MUTE_MINUTES} мин", bcid)
                return
                ...
                mutes.pop(t, None)
                await try_delete(message)
                await send_business_reply(message, "✅ Предупреждения и мут сброшены", bcid)
                return
                ...
                if len(p) >= 3:
                    ...
                    for _ in range(n):
                        try:
                            await send_business_reply(message, p[2], bcid)
                ...
                if text:
                    ...
                    for word in text.split():
                        try:
                            await send_business_reply(message, word, bcid)
                            await asyncio.sleep(0.2)
                        except Exception:
                            await asyncio.sleep(0.4)
                return
                ...
                await try_delete(message)
                if text:
                    await send_business_reply(message, text, bcid)
                return

            elif cmd == ".roll":
                n = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else 100
                await try_delete(message)
                await send_business_reply(message, f"🎲 {random.randint(1, n)}", bcid)
                return

            elif cmd == ".flip":
                await try_delete(message)
                await send_business_reply(message, random.choice(["🦅 Орёл", "🪙 Решка"]), bcid)
                return
                ...
                try:
                    ...
                    res = "ошибка"
                await try_delete(message)
                await send_business_reply(message, f"🧮 {expr} = {res}", bcid)
                return
                ...
                clone[t] = (state == "on")
                await try_delete(message)
                await send_business_reply(message, f"🔄 Автоповтор {'вкл' if state == 'on' else 'выкл'}", bcid)
                return
                ...
                await try_delete(message)
                if state == "off":
                    await send_business_reply(message, "🔊 Тихий режим выкл", bcid)
                return

            elif cmd == ".stats":
                s = get_stats(t)
                await try_delete(message)
                await send_business_reply(message, f"📊 Варнов: {warns.get(t, 0)}/{WARN_LIMIT}\nМут: {'да' if t in mutes else 'нет'}\nУдалено: {s['deleted']}", bcid)
                return

            elif cmd == ".info":
                await try_delete(message)
                await send_business_reply(message, f"🆔 Чат: {t}\nВладелец: {owner_id}\nТвой ID: {msg_from}\nВ кэше: {len(message_cache.get(t, {}))}", bcid)
                return
        ...
        if clone.get(t) and message.text and not is_owner:
            await send_business_reply(message, message.text, bcid)
