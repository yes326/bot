...
# ================== ЛИЧНЫЕ СООБЩЕНИЯ (МЕНЮ И КНОПКИ) ==================
...
async def handle_commands(message: types.Message):
    ...
    if cmd == ".mute":
        ...
        for mid, d in items:
            text += f"({d['time']}): {d['text'][:150]}\n"
        await message.answer(text[:4000])

# ================== ХЭНДЛЕРЫ ДЛЯ ЛС И TELEGRAM BUSINESS ==================
@dp.message(F.text.startswith("."))
async def on_private_command(message: types.Message):
    """Обработка команд с точкой в личных сообщениях"""
    await handle_commands(message)

@dp.business_message(F.text.startswith("."))
async def on_biz_command(message: types.Message):
    """Обработка команд с точкой в Telegram Business"""
...
@dp.message()
async def on_pm_message(message: types.Message):
    t = message.chat.id
...
async def on_biz_message(message: types.Message):
    ...
    if t in mutes and mutes[t] <= datetime.now():
        ...
        if clone.get(t) and message.text and msg_from != owner_id:[span_2](start_span)[span_2](end_span)
        await message.answer(message.text)[span_3](start_span)[span_3](end_span)

# ================== ЗАПУСК ==================[span_4](start_span)[span_4](end_span)
async def main():[span_5](start_span)[span_5](end_span)
    me = await bot.get_me()[span_6](start_span)[span_6](end_span)
    bot.username = me.username[span_7](start_span)[span_7](end_span)
    logging.info(f"✅ Бот успешно запущен: @{me.username}")[span_8](start_span)[span_8](end_span)
    await dp.start_polling(bot, drop_pending_updates=True)[span_9](start_span)[span_9](end_span)

if __name__ == "__main__":[span_10](start_span)[span_10](end_span)
