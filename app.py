...
BANNER_PATH = os.path.join(os.path.dirname(__file__), "1000085415.jpg")
...
async def send_photo_or_text(target, caption, reply_markup=None, parse_mode=None):
    """Вспомогательная функция для отправки сообщений с картинкой"""
    try:
...
async def b_commands(message: types.Message):
    ...
    if cmd == ".mute":
        ...
        for _ in range(n):
            ...
            await asyncio.sleep(0.05)  # Минимальная пауза 0.05 сек для обхода жесткого FloodWait
