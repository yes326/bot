...
async def cb_pay(call: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"paid_{plan}")],
...
async def on_screenshot(message: types.Message):
    ...
    owner_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Да", callback_data=f"approve_{user.id}_{plan}")],
        [InlineKeyboardButton(text="❌ Нет", callback_data=f"reject_{user.id}")],
    ])
    try:
...
async def b_commands(message: types.Message):
    ...
    owner_id = await get_owner_id(message.business_connection_id) if message.chat.type != "private" else message.from_user.id
...
async def on_biz_message(message: types.Message):
    t = message.chat.id
    owner_id = await get_owner_id(message.business_connection_id) if message.chat.type != "private" else OWNER_ID
